...
- 严格 runtime evidence critic 仍未通过：缺少 `REAL_LLM_3_MODES_PASS` 和 `INSERT_CLAUDE_PASS`；Claude 现有 Chrome 登录态已确认可打开输入框，但 Smart Prompt 扩展未加载到该页签，且未获得关闭/重启当前 Chrome 以启用 CDP 的确认。
- 用户确认重启 Chrome 跑 Claude 验收后，已重启默认 Chrome profile 并确认 `https://claude.ai/new` 登录态恢复、输入框可用；但新版 Chrome 对默认 profile 不开放可用 CDP 端口，且 Browser Use 安全策略阻止访问 `chrome://extensions` 加载 unpacked 扩展，因此不能自动把 Smart Prompt 扩展加载进默认登录态。已打开专用 CDP profile `.runtime/v2-live-chrome-profile` 到 Claude，端口 `9232` 可用，但该窗口仍在 Claude 登录页，需要用户在该专用窗口登录后才能生成正式 `INSERT_CLAUDE_PASS` 报告。
